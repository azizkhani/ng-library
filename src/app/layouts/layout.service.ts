...
import {NotificationService} from '../shared/utils/saUtils/notification.service';
...
const store = {
    ...
    skins: config.skins,
    /* fixedHeader: localStorage.getItem('sm-fixed-header') == 'false',*/
...
export class LayoutService {
    ...
    onLeftBarToggle(condition?: any) {
        if (condition == null) {
            this.store.leftBarOpen = !this.store.leftBarOpen;
        } else {
            this.store.leftBarOpen = !!condition;
        }
    ...
    processBody(state) {
        let $body = $('body');
        let $navSearchArea = $('.nav-search-area');
        ...
        $body.toggleClass('shortcut-on', state.leftBarOpen);
        ...
        if (state.menuMinified && !state.menuOnTop && !state.mobileViewActivated) {
            ...
            $navSearchArea.addClass('hidden');
        } else {
            $navSearchArea.removeClass('hidden');
        }
    }
}
